...
class PPIModel(BaseModel):
    ...
    def from_config(cls, cfg):
        ...
        return {"embedding_dim": cfg.model.embedding_dim, "num_layers": cfg.model.num_layers, "num_heads": cfg.model.num_heads, "token_dropout": cfg.model.token_dropout,
                "use_crp_embeddings": cfg.model.use_crp_embeddings, "alphabet": alphabet}

    @configurable
    def __init__(self, embedding_dim=1280, num_layers=33, num_heads=20, token_dropout=True, use_crp_embeddings=True, alphabet=None):
        super().__init__()
        if alphabet is None:
        ...
        self.model = ESM2(num_layers=num_layers, embed_dim=embedding_dim, attention_heads=num_heads, alphabet=self.alphabet, token_dropout=token_dropout,
    ...
    def _parse_inputs(self, inputs):
        ...
        input_dict = {'tokn-o': tokn_mat_orig, 'tokn-p': tokn_mat_pert, 'mask': mask_mat, 'asym': asym_ids, 'enty': enty_ids, }
    ...
    def _forward_impl(self, inputs):
        ...
        * <asym>, <enty>, and <mask> can be generated w/ utility functions in utils.py.
        ...
        results = self.model(input_dict['tokn-p'], asym_ids=input_dict['asym'], enty_ids=input_dict['enty'], repr_layers=self.repr_layers, need_head_weights=rtn_sp_feats,
                             return_contacts=False)

        # pack all the tensors into a dict
        outputs = {'labl': input_dict['tokn-o'], 'mask': input_dict['mask'], 'pred': results['logits'], }
    ...
    def forward(self, sequences: T.List[str], mask_prob: float = 0.0, mask_vecs: T.Optional[torch.Tensor] = None):
        ...
        """
        raw_data = {'seqs': sequences, 'asym': get_asym_ids(sequences), 'enty': get_entity_ids(sequences),
