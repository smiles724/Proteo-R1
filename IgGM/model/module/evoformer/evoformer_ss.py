...
class EvoformerBlockSS(nn.Module):
    ...
    def forward(self, s, z, chunk_size=None, qkv_data=None):
        """
        Args:
            s: [N, L, c_s], single features
            z: [N, L, L, c_z], pair features
        ...
        z = z + self.pair_dropout_row(self.tri_att_start(z, chunk_size=chunk_size, qkv_data=qkv_data))
        z = z + self.pair_dropout_col(self.tri_att_end(z, chunk_size=chunk_size, qkv_data=qkv_data))
    ...
    def forward(self, s, z, chunk_size=None, qkv_data=None):
        ...
        for block in self.blocks:
            if not (self.training and self.activation_checkpoint):
                s, z = block(s, z, chunk_size, qkv_data)
            else:
                s, z = self.activation_checkpoint_fn(block, s, z, chunk_size, qkv_data)
