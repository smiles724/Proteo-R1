...
def antibody_design_trunk(task="design"):
    print("Loading Antibody Design Model")
    if task == "design" or task == "affinity_maturation":
        # For antibody design, we use the trunk model
        return load_model_hub("antibody_design_trunk")
    elif task == "inverse_design":
        # For inverse design, we use the inverse design model
        return load_model_hub("antibody_inverse_design_trunk")
    elif task == "fr_design":
        # For framework design, we use the framework design model
        return load_model_hub("antibody_fr_design_trunk")
    else:
        raise ValueError(f"Unknown task: {task}. Supported tasks are 'antibody_design', 'inverse_design', and 'fr_design'.")
...
def _download_model_data(model_name):
    url = f"https://zenodo.org/records/16909543/files/{model_name}.pth?download=1"
