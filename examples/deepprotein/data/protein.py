...
class FLIPDataset(data.ProteinDataset):
    def load_csv(self, csv_file, sequence_field="sequence", target_fields=None, verbose=0, **kwargs):
        ...
        with open(csv_file) as fin:
            ...
            if verbose:
                reader = iter(tqdm(reader, f"Loading {csv_file}", utils.get_line_count(csv_file)))
            fields = next(reader)
            train, valid, test = [], [], []
            _sequences = []
        ...
        valid_set = set(valid)
        sequences = [_sequences[i] for i in train if i not in valid_set] + [_sequences[i] for i in valid] + [_sequences[i] for i in test]
        targets = defaultdict(list)
        for key, value in _targets.items():
            targets[key] = [value[i] for i in train if i not in valid_set] + [value[i] for i in valid] + [value[i] for i in test]
...
class AAV(FLIPDataset):
    ...
    def __init__(self, path, split="two_vs_many", keep_mutation_region=False, verbose=1, **kwargs):
        ...
        csv_file = os.path.join(data_path, f"splits/{split}.csv")
