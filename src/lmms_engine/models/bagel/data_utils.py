# Adapted from https://github.com/ByteDance-Seed/Bagel/blob/main/data/data_utils.py
...
def patchify(image, patch_size):
    p = patch_size
    c, h, w = image.shape
    assert h % p == 0 and w % p == 0, f"Image size {h}x{w} is not divisible by patch size {p}"
