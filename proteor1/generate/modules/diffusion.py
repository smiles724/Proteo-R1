...
# Configure logging level when DEBUG_TENSORS is enabled
if DEBUG_TENSORS:
    logging.basicConfig(level=logging.INFO, format='%(message)s')
    _logger.setLevel(logging.INFO)


def _log_diffusion_inputs(gt_vals, seq_masks, cdr_masks, masked_seq, prefix: str = ""):
    """
    Log diffusion sample method inputs for comparison.

    Parameters
    ----------
    gt_vals : Tensor
        Ground truth values for sequence.
    seq_masks : Tensor
        Sequence mask.
    cdr_masks : Tensor
        CDR mask.
    ...
    tag = f"[DEBUG-MFD] {prefix}" if prefix else "[DEBUG-MFD]"
    _logger.info(f"{tag} ========== Diffusion Sample Inputs ==========")
    ...
    if gt_vals is not None:
        arr = _tensor_to_np(gt_vals)
        unk_mask = (arr == 22)
        unk_count = unk_mask.sum()
        unk_indices = np.where(unk_mask.flatten())[0][:10].tolist()
        _logger.info(f"{tag} gt_vals: UNK(22) count={unk_count}, first 10 UNK indices={unk_indices}")

    if seq_masks is not None:
        arr = _tensor_to_np(seq_masks).astype(bool)
        true_count = arr.sum()
        true_indices = np.where(arr.flatten())[0][:10].tolist()
        _logger.info(f"{tag} seq_masks: True count={true_count}, first 10 True indices={true_indices}")

    if cdr_masks is not None:
        arr = _tensor_to_np(cdr_masks).astype(bool)
        true_count = arr.sum()
        true_indices = np.where(arr.flatten())[0][:10].tolist()
        _logger.info(f"{tag} cdr_masks: True count={true_count}, first 10 True indices={true_indices}")

    if masked_seq is not None:
        arr = _tensor_to_np(masked_seq)
        unk_mask = (arr == 22)
        unk_count = unk_mask.sum()
        unk_indices = np.where(unk_mask.flatten())[0][:10].tolist()
        _logger.info(f"{tag} masked_seq (input): UNK(22) count={unk_count}, first 10 UNK indices={unk_indices}")

    _logger.info(f"{tag} ========== End Diffusion Sample Inputs ==========")
from proteor1.generate.loss.diffusion import (
    smooth_lddt_loss,
    weighted_rigid_align,
)
from proteor1.generate.data_load.mask.masker import Masker
from proteor1.generate.modules.encoders import (
...
from proteor1.generate.modules.transformers import (
...
from proteor1.generate.modules.trunk import InputEmbedder
    ...
    def sample(
        ...
        **network_condition_kwargs,
        ...
        if self.sequence_train:
            ...
            # During training (forward), seq_mask is dynamically generated by masker.corrupt()
            # as a random subset of cdr_mask based on noise level.
            # During inference (sample), seq_mask represents "regions to generate", which is
            # the entire CDR region. Therefore, using cdr_mask as fallback is semantically correct.
            seq_masks = network_condition_kwargs["feats"].get("seq_mask", cdr_masks)

            # Debug logging: log inputs before corruption
            _log_diffusion_inputs(
                gt_vals, seq_masks, cdr_masks,
                network_condition_kwargs["feats"]["masked_seq"],
                prefix="Before corruption"
            )
