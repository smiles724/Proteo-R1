...
class PLLMForCausalLM(nn.Module):
    ...
    def __init__(self, config=None, **kwargs):
        ...
        for attr_name in ["llm", "protein_encoder", "structure_encoder", "prefix_mlp", "tokenizer", "hidden_size", "prefix_len", "protein_config", "structure_config", "train_encoders", "proj_hid"]:
    ...
    def from_pretrained(cls, pretrained_model_name_or_path, config=None, **kwargs):
        ...
        print("[PLLMForCausalLM] Imported PLLM class successfully")
        ...
        print("[PLLMForCausalLM] PLLM model loaded successfully")
        ...
        if config is None:
            ...
            print("[PLLMForCausalLM] Config loaded successfully")

        # Create wrapper instance without calling __init__
        print("[PLLMForCausalLM] Creating wrapper instance")
        wrapped_model = cls.__new__(cls)

        # Initialize nn.Module parent class
        print("[PLLMForCausalLM] Initializing nn.Module parent class")
        nn.Module.__init__(wrapped_model)

        # Copy all PLLM attributes
        print("[PLLMForCausalLM] Copying PLLM attributes")
        for attr_name in ["llm", "protein_encoder", "structure_encoder", "prefix_mlp", "tokenizer", "hidden_size", "prefix_len", "protein_config", "structure_config", "train_encoders", "proj_hid"]:
            if hasattr(pllm_model, attr_name):
        ...
        # Copy methods
        print("[PLLMForCausalLM] Copying PLLM methods")
        for method_name in ["encode_protein_batch", "forward", "generate", "save_pretrained"]:
            if hasattr(pllm_model, method_name):
        ...
        print("[PLLMForCausalLM] Set config successfully")

        print("[PLLMForCausalLM] from_pretrained completed successfully")
    ...
    def named_parameters(self, prefix="", recurse=True, remove_duplicate=True):
        ...
        if not recurse:
            ...
            print("[PLLMForCausalLM] recurse=False, getting parameters from direct children")
            ...
            print("[PLLMForCausalLM] recurse=True, getting parameters from all submodules")
            for name, module in [("llm", getattr(self, "llm", None)), ("protein_encoder", getattr(self, "protein_encoder", None)), ("structure_encoder", getattr(self, "structure_encoder", None)), ("prefix_mlp", getattr(self, "prefix_mlp", None))]:
...
# Also register vLLM adapter if available to satisfy vLLM backend checks
try:
    from rllm.model import vllm_pllm as _vllm_pllm  # noqa: F401
except Exception:
    pass
