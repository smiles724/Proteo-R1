...
def prepare_rlvr_data(dataset_name="thermostability"):
    ...
    def preprocess_fn(example, idx):
        return {
            ...
            "aa_seq": example["sequence"],
            "split": example["set"],
            "ground_truth": example["target"],
        }

    dataset = dataset.map(preprocess_fn, with_indices=True)
    train_dataset = dataset["train"].filter(lambda x: x["split"] != "test")
...
def prepare_sft_data():
    ...
    def preprocess_fn(example, idx):
        return {
            "prompt": example.get("prompt", ""),
            "aa_seq": example["aa_seq"],
            "stru_str": example["stru_str"],
            "ground_truth": example["description"],
