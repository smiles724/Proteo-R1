...
class AbDesigner(BaseDesigner):
    ...
    def __init__(self, ppi_path, design_path, buffer_path, config):
        ...
        self.understanding_model = ProteinLLM.restore(understanding_path)  # load the understanding model
    ...
    def forward(self, inputs, chunk_size=None, temperature=1.0):
        ...
        # extract qkv from understanding model   # todo: use the understanding model to extract qkv
        input_ids = inputs['base']['input_ids']
        attention_mask = inputs['base']['attention_mask']
        labels = inputs['base']['labels']
        aa_sequences = inputs['base']['aa_sequences']
        structure_strings = inputs['base']['structure_strings']
        output = self.understanding_model.forward_with_qkv_extraction(
            input_ids=input_ids,
            attention_mask=attention_mask, 
            labels=labels,
            aa_seq=aa_sequences,
            stru_str=structure_strings,
            extract_qkv=True  # This triggers QKV extraction
        )

        # Extract QKV from the cached layer
        qkv_data = self.understanding_model.get_qkv_from_layer(layer_idx=-1)
        ...
        for idx_step in idxs_step:
            ...
            aa_seqs_pred, cord_tns_pred, inputs_addi = \
                self.__sample_cm_ss2ss(prot_data_curr, idx_step, inputs_addi, chunk_size=chunk_size,
                                       temperature=temperature, qkv_data=qkv_data)
    ...
    def __sample_cm_ss2ss(self, prot_data_curr, idx_step, inputs_addi, chunk_size=None, temperature=1.0, qkv_data=None):
        """Sample amino-acid sequences & backbone structures w/ CM."""

        # perform the forward pass
        inputs = self.__build_inputs_cm(prot_data_curr, idx_step)
        outputs = self.model(inputs, inputs_addi=inputs_addi, chunk_size=chunk_size, qkv_data=qkv_data)
