...
DTYPE_MAP = {"fp32": torch.float32, "float32": torch.float32, "fp16": torch.float16, "float16": torch.float16, "bf16": torch.bfloat16, "bfloat16": torch.bfloat16, "auto": None, "default": None, None: None}
...
class PLLM(nn.Module):
    ...
    def _export_config(self):
        ...
        return {
            ...
            "auto_map": {"AutoConfig": "register_pllm.PLLMConfig", "AutoModelForCausalLM": "register_pllm.PLLMForCausalLM", "AutoModel": "register_pllm.PLLMForCausalLM"},
            "base_model_name_or_path": getattr(self.llm, "name_or_path", None),
            "hidden_size": int(getattr(self, "hidden_size", 0)),
            "prefix_len": int(getattr(self, "prefix_len", 1)),
    ...
    def _construct_from_config(cls, cfg: dict, base_path: str = None):
        ...
        return cls(
            ...
            protrek_ckpt=None,  # Weights loaded from safetensors
            single_token_prefix=cfg.get("single_token_prefix", False),
            prefix_len=int(cfg.get("prefix_len", 4)),
            proj_hid=int(cfg.get("proj_hid", 1024)),
            train_encoders=bool(cfg.get("train_encoders", True)),
            dtype_str="auto",
        )
