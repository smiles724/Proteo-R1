...
def sequence_identity(gt: str, pred: str, k=2.0, alpha=0.8):
    ...
    alignments = pairwise2.align.globalds(gt, pred, blosum62, -11, -1)  # gap_open, gap_extend
    aln_gt, aln_pred, score, start, end = alignments[0]

    # Identity fraction
    matches = sum(a == b for a, b in zip(aln_gt, aln_pred, strict=False) if a != "-" and b != "-")
    length = len(aln_gt)
    R_id = matches / length
...
class RewardProteinFn:
    ...
    def __call__(self, task_info: dict, action: str) -> RewardOutput:
        ...
        for ground_truth in processed_ground_truths:
            if self.config.bio_task == "function":
            ...
            elif self.config.bio_task == "inverse_folding":
                aar = grade_answer_IF(model_answer, ground_truth)
                assert 0.0 <= aar <= 1.0
                return RewardOutput(reward=aar, is_correct=aar == 1.0)

            elif self.config.bio_task == "property_prediction":
                # For property prediction (e.g., thermostability), compare numerical values
                is_correct = grade_answer_mathd(model_answer, ground_truth) or grade_answer_sympy(model_answer, ground_truth)
                if is_correct:
                    # Apply tool call bonus if applicable and answer is correct
                    reward = self.config.correct_reward
