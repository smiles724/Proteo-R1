...
def train_agent(config, agent_class=None, env_class=None, agent_args=None, env_args=None):
    ...
    if config.actor_rollout_ref.actor.strategy in ["fsdp", "fsdp2"]:   # no more megatron!
    ...
    reward_fn = load_reward_manager(config, tokenizer, num_examine=0, **config.reward_model.get("reward_kwargs", {}))   # todo: modify the reward model
