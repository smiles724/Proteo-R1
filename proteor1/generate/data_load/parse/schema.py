...
def parse_boltz_schema(  # noqa: C901, PLR0915, PLR0912
            ...
            for atom in res.atoms:
                ...
                atom_data.append(
                    (
                        ...
                        atom.element,
                        # element is the atomic number of the atom, i.e. its position in the periodic table
                        atom.charge,
                        # charge is the formal charge of the atom
                        atom.coords,
    ...
    # Calculate H chain length for CDR JSON output
    # chains is already a numpy structured array at this point, so iterate directly
    h_chain_length = None
    if h_chain_name is not None:
        for chain in chains:
            if chain["name"] == h_chain_name:
                entity_id = int(chain["entity_id"])
                if entity_id in entity_to_seq:
                    h_chain_length = len(entity_to_seq[entity_id])
                break
