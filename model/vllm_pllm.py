...
try:
    ...
    from vllm.multimodal.inputs import MultiModalKwargs, MultiModalFieldConfig
...
if VLLM_AVAILABLE:
    ...
    class PLLMDummyInputsBuilder(BaseDummyInputsBuilder[PLLMProcessingInfo]):
        ...
        def get_dummy_processor_inputs(self, seq_len: int, mm_counts: Mapping[str, int]) -> dict:
            dummy_inputs = {}
            if mm_counts.get("protein_sequence", 0) > 0:
                dummy_inputs["protein_sequence"] = ["MKT"]
            if mm_counts.get("structure_data", 0) > 0:
                dummy_inputs["structure_data"] = ["ABC"]
            return dummy_inputs
    ...
    class VLLMPLLMForCausalLM(nn.Module, SupportsMultiModal):
        ...
        def __init__(self, *, vllm_config: VllmConfig, prefix: str = ""):
            ...
            base_model_path = getattr(self.hf_config, "base_model_name_or_path")
            llm_vllm_config = copy.deepcopy(vllm_config)
            llm_vllm_config.model_config.hf_config = AutoConfig.from_pretrained(
                base_model_path, trust_remote_code=vllm_config.model_config.trust_remote_code
            )
            ...
            self.sub_modules = nn.ModuleDict({
                'language_model': Qwen2ForCausalLM(vllm_config=llm_vllm_config)
            })
            ...
            self.protein_encoder = (
                ProteinEncoder(protein_cfg, out_dim=1024, load_pretrained=False)
                if protein_cfg is not None else None
            )
            self.structure_encoder = (
                StructureEncoder(structure_cfg, out_dim=1024, load_pretrained=False)
                if structure_cfg is not None else None
            )
        ...
        @torch.no_grad()
        def get_multimodal_embeddings(self, **kwargs: object) -> Optional[tuple[torch.Tensor, ...]]:
        ...
        def get_input_embeddings(self, input_ids: torch.Tensor, multimodal_embeddings: Optional[tuple[torch.Tensor, ...]] = None, attn_metadata: Optional["IntermediateTensors"] = None) -> torch.Tensor:
...
def register_vllm_pllm():
    ...
    try:
        ...
        print(f"✅ Registered VLLMPLLMForCausalLM with vLLM ModelRegistry for key 'PLLMForCausalLM'")
