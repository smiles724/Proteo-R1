...
class GatedMultiheadAttention(nn.Module):
    ...
    def __init__(
            ...
            *,
            ...
            num_heads: int = 8,
            gating: bool = True,
            external_qkv_dim=None, # dimension of the external qkv
    ):
        ...
        if external_qkv_dim:
            self.external_q_proj = Linear(external_qkv_dim, self.head_dim * self.num_heads)
            self.external_k_proj = Linear(external_qkv_dim, self.head_dim * self.num_heads) 
            self.external_v_proj = Linear(external_qkv_dim, self.head_dim * self.num_heads)
    ...
    def _deepspeed_evo_attn(
            ...
            external_qkv: torch.Tensor = None, # external qkv data
    ):
        ...
        if external_qkv is not None:
            external_q = self.external_q_proj(external_qkv['q'])
            external_k = self.external_k_proj(external_qkv['k'])
            external_v = self.external_v_proj(external_qkv['v'])
            external_q = reshape_dims(external_q)
            external_k = reshape_dims(external_k)
            external_v = reshape_dims(external_v)

            q = torch.cat([q, external_qkv], dim=-1)
            k = torch.cat([k, external_qkv], dim=-1)
            v = torch.cat([v, external_qkv], dim=-1)
    ...
    def _scale_dot_product_attention(
        ...
        if qkv_data is not None:
            query = torch.cat([query, qkv_data['q']], dim=-1)
            key = torch.cat([key, qkv_data['k']], dim=-1)
            value = torch.cat([value, qkv_data['v']], dim=-1)
            attn_mask = torch.cat([attn_mask, qkv_data['m']], dim=-1)
    ...
    def forward(
        ...
        if use_ds_attn and self.use_evo_attn:   # not using deepspeed attention! 
            y = self._deepspeed_evo_attn(q, k, v, biases, qkv_data)
        else:
