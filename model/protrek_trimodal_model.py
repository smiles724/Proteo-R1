...
def multilabel_cross_entropy(logits, labels):
    ...
    loss = 0
    for pred, label in zip(logits, labels, strict=False):
...
class ProTrekTrimodalModel(AbstractModel):
    def __init__(self, protein_config: str, text_config: str, structure_config: str = None, repr_dim: int = 1024, temperature: float = 0.07, load_protein_pretrained: bool = True, load_text_pretrained: bool = True, use_mlm_loss: bool = False, use_zlpr_loss: bool = False, use_saprot: bool = False, gradient_checkpointing: bool = False, **kwargs):
        """
        Args:
    ...
    def initialize_model(self):
        # Initialize encoders
        self.protein_encoder = ProteinEncoder(self.protein_config, self.repr_dim, self.load_protein_pretrained, self.gradient_checkpointing)

        self.text_encoder = TextEncoder(self.text_config, self.repr_dim, self.load_text_pretrained, self.gradient_checkpointing)

        # Learnable temperature
        self.temperature = torch.nn.Parameter(torch.tensor(self.temperature))

        # self.model is used for saving and loading
        self.model = torch.nn.ParameterList([self.temperature, self.protein_encoder, self.text_encoder])
    ...
    def loss_func(self, stage: str, outputs, labels):
        ...
        _ = self.all_gather(protein_repr).view(-1, protein_repr.shape[-1]).detach()
        _ = self.all_gather(text_repr).view(-1, text_repr.shape[-1]).detach()

        if self.structure_config is not None:
            structure_repr = normalize(structure_repr, dim=-1)
            _ = self.all_gather(structure_repr).view(-1, structure_repr.shape[-1]).detach()
        ...
        if self.structure_config is not None:
            pairs = {"protein": ["structure", "text"], "structure": ["protein", "text"], "text": ["protein", "structure"]}
        else:
    ...
    def _get_protein_indices(self):
        ...
        if self.use_saprot:
            ...
            for sub_dict in self.uniprot2label.values():
                ...
                seq = "".join([a + b for a, b in zip(aa_seq, foldseek_seq, strict=False)])
                proteins.append(seq)
    ...
    def _protein2text(self, modality: str, protein_indices, text_indices: dict):
        ...
        mpr = MultipleProcessRunnerSimplifier(sub_inputs, do, save_path=tmp_path, n_process=8, verbose=verbose, return_results=True)
        outputs = mpr.run()
        os.remove(tmp_path)
        ...
        for input, output in zip(inputs, tensor_outputs, strict=False):
            ap, mrr, auc = output
        ...
        for level, labels in [("residue-level", residue_level), ("sequence-level", sequence_level), ("all", residue_level | sequence_level)]:
            mrrs = []
            maps = []
            aucs = []
    ...
    def _load_eval_data(self, stage):
        ...
        self.uniprot2label = json.load(open(uniprot2label_path))
        self.label2text = json.load(open(label2text_path))
