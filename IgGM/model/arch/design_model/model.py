...
from IgGM.model.layer.embedding import SinusoidalPositionEmbedding, RelativePositionEmbedding, StructEncoder, RcEmbedNet
...
class DesignModel(BaseModel):
    ...
    def __init__(self, n_dims_sfea_init=-1,  # number of dimensions in initial single features (D_si)
    ...
    def featurize(cls, plm_featurizer, prot_data):
        ...
        * inputs: dict of input tensors
        ...
        if asym_id.max() == 1:
            ...
            inputs = {**prot_data, 'sfea-i': sfea_mat,  # N x L x D_s
            ...
            pfea_tns[:, -ag_pfea_tns.shape[1]:, -ag_pfea_tns.shape[1]:, :] = ag_pfea_tns
            del ab_sfea_mat_list, ab_pfea_tns_list, ag_sfea_mat_list, ag_pfea_tns_list
            inputs = {**prot_data, 'sfea-i': sfea_mat,  # L x D_s
    ...
    def forward(self, inputs, inputs_addi=None, chunk_size=None, qkv_data=None):
        ...
        * If the input dict of additional protein data corresponds to $x_{t + 1}$, then it should
        ...
        if inputs_addi is None:  # no additional inputs
            ...
            outputs = self.__forward_impl(inputs, chunk_size=chunk_size, qkv_data=qkv_data)
        else:
            ...
            if all(x == 0 for x in inputs_addi['step']):  # $\hat{x}_{0}$
                ...
                with torch.no_grad():
                    ...
                    outputs = self.__forward_impl(inputs_addi, qkv_data=qkv_data)
                inputs_sc = {'sfea': outputs['sfea'].detach(), 'pfea': outputs['pfea'].detach(), 'logt': outputs['1d'].permute(0, 2, 1).detach(),  # N x L x C
            ...
            self.net['evoformer'].requires_grad_(self.training)
            outputs = self.__forward_impl(inputs, inputs_sc=inputs_sc, chunk_size=chunk_size, qkv_data=qkv_data)

        return outputs

    def __forward_impl(self, inputs, inputs_sc=None, chunk_size=None, qkv_data=None):
        ...
        ligand_feat = {'seq': inputs['seq-p'][0][:ab_len], 'cord': inputs['cord-p'][0][:ab_len, ...], }
        receptor_feat = {'seq': inputs['seq-p'][0][-a_len:], 'cord': inputs['cord-p'][0][-a_len:, ...], }
        ...
        if inputs_sc is not None:
            rc_inputs = {'sfea': inputs_sc['sfea'] + self.net['linear-lt-sd'](inputs_sc['logt']), 'pfea': inputs_sc['pfea'], 'cord': inputs_sc['cord'], }
            mfea_tns, pfea_tns = self.net['rc_embed-sd'](sfea_tns.unsqueeze(dim=1), pfea_tns, rc_inputs=rc_inputs)
            sfea_tns = mfea_tns[:, 0]

        # Evoformer
        sfea_tns, pfea_tns = self.net['evoformer'](sfea_tns, pfea_tns, chunk_size=chunk_size, qkv_data=qkv_data)

        # AF2SMod
        sfea_tns_st, cord_list, param_list, plddt_list, _ = self.net['af2_smod'](inputs['seq-p'], sfea_tns, pfea_tns, penc_tns, cord_tns_init=inputs['cord-p'],
                                                                                 cmsk_tns_init=inputs['cmsk-p'], rmsk_vec_motf=rmsk_vec_motf, qkv_data=qkv_data)
        ...
        outputs = {'sfea': sfea_tns, 'mask': inputs['pmsk'], 'pfea': pfea_tns, '1d': logt_tns_aa,
                   '2d': {'cb': logt_tns_cb, 'om': logt_tns_om, 'th': logt_tns_th, 'ph': logt_tns_ph}, '3d': {'cord': cord_list, 'param': param_list, 'plddt': plddt_list}, }
        return outputs
    ...
    def __build_model(self):
        ...
        # Evoformer
        net['evoformer'] = EvoformerStackSS(num_layers=self.n_lyrs_2d, c_s=self.n_dims_sfea, c_z=self.n_dims_pfea, )

        # AF2SMod
        net['af2_smod'] = StructureModule(n_lyrs=self.n_lyrs_3d, n_dims_sfea=self.n_dims_sfea, n_dims_pfea=self.n_dims_pfea, n_dims_encd=self.n_dims_penc, pred_oxyg=self.pred_oxyg,
                                          pred_schn=False, )
        ...
        net['da_pred'] = PairPredictor(c_z=self.n_dims_pfea, bins=[37, 25, 25, 25], )

        # RcEmbedNet (for self-conditioning inputs)
        net['linear-lt-sd'] = nn.Linear(len(RESD_NAMES_1C), self.n_dims_sfea)
        net['rc_embed-sd'] = RcEmbedNet(n_dims_mfea=self.n_dims_sfea, n_dims_pfea=self.n_dims_pfea, )
