...
import os
import random
from os.path import dirname, abspath, join, splitext, relpath
...
async def main():
    ...
    parser.add_argument('--model_id', type=str, default="grok-4")
    ...
    result_save_path = join(
        RESULT_ROOT,
        reformat_model_id,
        f"{args.n_sampling}_{args.max_tokens}",
                ...
                for task in tqdm_asyncio.as_completed(tasks, total=len(tasks)):
                    ...
                    if ret_dict["responses"] is not None:
                        for single_response in ret_dict["responses"].choices:
                            try:
                                generated_text = single_response.message.content
